...
while True:
    ...
    try:
        ...
        # Draw a rectangle and put a label on the original image
        top_left = (50, 50)  # Example coordinates
        bottom_right = (200, 200)  # Example coordinates
        # Draw rectangle on the original image
        cv2.rectangle(image, top_left, bottom_right, (0, 255, 0), 2)
        # Add text label on the original image
        label_text = f"{class_name} ({np.round(average_confidence * 100, 2)}%)"
        cv2.putText(image, label_text, (top_left[0], top_left[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
    except Exception as e:
